...
api.interceptors.response.use(
    (response: AxiosResponse) => {
        const result = response.data as Respuesta<unknown>;
        return { ...response, data: result };
    },
    (error: unknown) => {
        // 2. Captura errores del GlobalExceptionHandler de .NET (HTTP 400, 401, 500, etc.) o fallas de red
        let formatoErrorGlobal: Respuesta<null> = {
            esExitoso: false,
            datos: null,
            detalleError: {
                codigo: 'UNKNOWN_ERROR',
                descripcion: 'Ocurrió un error inesperado en la comunicación con el servidor.',
            }
        };
        ...
        if (error instanceof AxiosError) {
            ...
            if (error.response?.data && 'isSuccess' in error.response.data) {
                formatoErrorGlobal = error.response.data as Respuesta<null>;
                ...
                formatoErrorGlobal.detalleError = {
                    ...
                    descripcion: `Error de red: ${error.message}`
                };
            }
        }
